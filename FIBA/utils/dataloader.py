...
#             transforms.ToTensor()])
        dataset = torchvision.datasets.CIFAR10('../../data/', train, transform, download=False)
#     elif opt.dataset == 'cifar10':
...
def get_dataset(opt, train=True):
    ...
    if opt.dataset == "gtsrb":
        ...
        transform = transforms.Compose([transforms.Resize((opt.input_height, opt.input_width)),
                                        transforms.Normalize([0.5], [0.5]),
                                       transforms.Lambda(lambda x: x.repeat(3, 1, 1)),
                                       ToNumpy()])
        dataset = torchvision.datasets.MNIST(opt.data_root, train, transform=transform, download=False)
    elif opt.dataset == "cifar10":
        transform = transforms.Compose([
            transforms.Pad(4),
            transforms.Normalize([0.4914, 0.4822, 0.4465], [0.247, 0.243, 0.261]),
            ...
            transforms.RandomCrop(32),
            ToNumpy()])
        dataset = torchvision.datasets.CIFAR10(opt.data_root, train, transform=transform, download=False)
