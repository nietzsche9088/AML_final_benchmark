...
def get_dataset(opt, train=True):
    if opt.dataset == "gtsrb":
        ...
        dataset = paddle.vision.datasets.MNIST(opt.data_root, train, transform=ToNumpy(), download=False)
    elif opt.dataset == "cifar10":
        dataset = paddle.vision.datasets.Cifar10(opt.data_root, train, transform=ToNumpy(), download=False)
    elif opt.dataset in ['tiny-imagenet', 'tiny-imagenet32']:
        if train:
            split = 'train'
        else:
