...
def get_dataset(opt, train=True):
    if opt.dataset == "gtsrb":
        ...
        dataset = torchvision.datasets.MNIST(opt.data_root, train, transform=ToNumpy(), download=False)
    elif opt.dataset == "cifar10":
        dataset = torchvision.datasets.CIFAR10(opt.data_root, train, transform=ToNumpy(), download=False)
    elif opt.dataset == "celeba":
        if train:
            split = "train"
        else:
